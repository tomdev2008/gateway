...
export WEB_APP_HOME=${yohobuy.order.env.webapphome}
export SERVER_PORT=${yohobuy.order.env.serverport}
export PRODUCTION_MODE="${yoho.env.productmode}"
export SERVER_HOME=${yoho.env.serverhome}
export SERVER_TYPE=${yoho.env.servertype}
export SERVER_NAMESPACE=${yohobuy.order.env.namespace}
export GW_URL=${Gateway_URL}

#if("${yoho.env.servertype}"=="jboss")
    export SERVER_BASE_DIR=" -Djboss.server.base.dir=${yohobuy.order.env.webapphome}/.jboss/default "
#end

#if("${yoho.env.servertype}"=="tomcat")
    export SERVER_BASE_DIR=" -Dcatalina.base=${yohobuy.order.env.webapphome}/.tomcat "
#end
